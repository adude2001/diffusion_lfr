...
def run_find_solution_optim(cur_z, final_result_traj, future_len, lr, loss_weights, model, 
    ...
    if tgt_prior_distrib is not None:
        tgt_z = tgt_prior_distrib[0]
    else:
        tgt_z = cur_z[tgt_mask]
    if len(tgt_z.size()) == 2:
        tgt_z = tgt_z.view(B, 1, -1)
    tgt_z = tgt_z.clone().detach()
    tgt_z.requires_grad = True

    other_z_all = cur_z[~tgt_mask]
    if len(other_z_all.size()) == 2:
        other_z_all = other_z_all.view(NA-B, 1, -1)
    other_z_all = other_z_all.clone().detach()
    ...
    for oidx in pbar_optim:
        def closure():
            ...
            tgt_future_pred = model.get_normalizer().unnormalize(tgt_decoder_out['future_pred'])
            if tgt_future_pred.dim() == 4:
                if tgt_future_pred.size(1) == 1:
                    tgt_future_pred = tgt_future_pred[:, 0]
                elif tgt_future_pred.size(0) == 1:
                    tgt_future_pred = tgt_future_pred[0]
                else:
                    tgt_future_pred = tgt_future_pred[:, 0]
