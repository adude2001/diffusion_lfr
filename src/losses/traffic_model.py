...
class TrafficModelLoss(nn.Module):
    ...
    def forward(self, scene_graph, pred,
        ...
        model_family = pred.get('model_family', 'cvae')

        kl_loss = None
        if model_family == 'diffusion':
            base_loss = pred['diffusion_loss']
            loss = base_loss if base_loss.dim() == 0 else base_loss.mean()
            recon_loss = loss.detach().reshape(1)
        else:
            # reconstruction loss
    ...
    def compute_err(self, scene_graph, pred, normalizer):
        ...
        pred_future = torch.nan_to_num(pred_future, nan=0.0, posinf=1e3, neginf=-1e3)
        ...
        gt_h = gt_future[:,2:4]
        gt_h = gt_h / torch.clamp(torch.norm(gt_h, dim=-1, keepdim=True), min=1e-6)
        pred_h = pred_future[:,2:4]
        pred_h = pred_h / torch.clamp(torch.norm(pred_h, dim=-1, keepdim=True), min=1e-6)
        ...
        if pred.get('posterior_out') is not None and pred.get('prior_out') is not None:
            post_mean = pred['posterior_out'][0]
            prior_mean, prior_var = pred['prior_out']
            prior_var = torch.clamp(prior_var, min=1e-6)
            z_logprob = log_normal(post_mean, prior_mean, prior_var)
            z_mdist = torch.norm((post_mean - prior_mean) / torch.sqrt(prior_var), dim=-1)
            err_out['z_logprob'] = z_logprob
            err_out['z_mdist'] = z_mdist
