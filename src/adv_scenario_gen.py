...
def parse_cfg():
    ...
    parser.add_argument('--adv_opt_space', type=str, default='z', choices=['z', 'noise', 'traj'],
                        help='Optimization variable space for adversarial generation.')
...
def run_one_epoch(data_loader, batch_size, model, map_env, device, out_path, loss_weights,
    ...
    for i, data in enumerate(pbar_data):
        ...
        try:
            ...
            if getattr(model, 'model_family', 'cvae') == 'diffusion':
                z_init = embed_info_attached['opt_init'].detach()
                ...
                z_init, init_fit_traj, _ = run_init_optim(z_init, init_traj, scene_graph.future_vis, 0.1, loss_weights, model,
                                                          scene_graph, map_env, map_idx, 75, embed_info, None)
            else:
            ...
            if planner_name == 'hardcode':
                ...
                init_prior = None if getattr(model, 'model_family', 'cvae') == 'diffusion' else embed_info['prior_out']
                z_init, init_fit_traj, _ = run_init_optim(z_init, init_traj, scene_graph.future_vis, lr, loss_weights, model,
                                                            scene_graph, map_env, map_idx, 100, embed_info, init_prior)
            ...
            if getattr(model, 'model_family', 'cvae') == 'diffusion':
                tgt_prior_distrib = None
                other_prior_distrib = None
            else:
            ...
            cur_opt_space = getattr(config, 'adv_opt_space', 'z')
            if getattr(model, 'model_family', 'cvae') == 'diffusion' and cur_opt_space == 'z':
                cur_opt_space = 'noise'
            ...
            if len(sol_graph_list) > 0:
                ...
                sol_tgt_prior_distrib = None if getattr(model, 'model_family', 'cvae') == 'diffusion' else (tgt_prior_distrib[0][sol_bmask], tgt_prior_distrib[1][sol_bmask])
                sol_other_prior_distrib = None if getattr(model, 'model_family', 'cvae') == 'diffusion' else (sol_embed_info['prior_out'][0][~sol_ego_mask], sol_embed_info['prior_out'][1][~sol_ego_mask])
            ...
            for b in range(B):
                ...
                if save:
                    ...
                    scene_out_dict = prepare_output_dict(scene_graph_list[b], map_idx[b].item(), map_env, data_loader.dataset.dt, model,
                                                          ...
                                                          out_sol_z,
                                                          (embed_info['prior_out'][0][scene_graph.ptr[b]:scene_graph.ptr[b+1]], embed_info['prior_out'][1][scene_graph.ptr[b]:scene_graph.ptr[b+1]]) if embed_info.get('prior_out') is not None else None,
                                                          internal_ego_traj=final_decoder_out['future_pred'][scene_graph.ptr[b]].detach(),
                                                          model_family=getattr(model, 'model_family', 'cvae'),
                                                          opt_space=cur_opt_space
                                                          )
...
def main():
    ...
    # create model
    model = build_traffic_model(cfg, map_env, len(test_dataset.categories)).to(device)
    ...
    if hasattr(cfg, 'use_llm') and cfg.use_llm:
        if not LLM_IMPORT_OK:
            Logger.log('LLM模块不可用（缺少src.llm或依赖），回退为默认静态权重')
        else:
            try:
