...
class GaussianDiffusionConditional(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, cond, inits, cond_scale=1., rescaled_phi=0.0, clip_denoised=True, reward_fn=None):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        
        # 1. Unet 预测均值 (mu) 和 方差 (variance)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, t=batched_times, cond=cond, inits=inits, 
            cond_scale=cond_scale, rescaled_phi=rescaled_phi, clip_denoised=clip_denoised
        )
        ...
        if reward_fn is not None:
            pop_size = 50       # 种群大小
            explore_scale = 1.0 # 探索步长放大系数
            
            std = torch.exp(0.5 * model_log_variance)
            noise_pop = torch.randn(pop_size, *model_mean.shape[1:], device=device)
            candidates = model_mean.repeat(pop_size, 1, 1) + noise_pop * std * explore_scale
            
            pred_x = torch.cumsum(candidates[:, 0, :], dim=-1) 
            pred_y = torch.cumsum(candidates[:, 1, :], dim=-1)
            trajs_xy = torch.stack([pred_x, pred_y], dim=1) 
            
            scores = reward_fn(trajs_xy) 
            
            k_elite = max(1, int(pop_size * 0.2)) # 选出前 20% 精英
            elite_indices = torch.topk(scores, k=k_elite).indices
            elites = candidates[elite_indices]
            
            # 核心：均值漂移！
            model_mean = elites.mean(dim=0, keepdim=True) 
        # ===================================================
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, inits, cond_scale=1., rescaled_phi=0.0, reward_fn=None):
        batch, device = shape[0], self.betas.device
        img = torch.randn(shape, device=device)

        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            # 将 reward_fn 往下传
            img, x_start = self.p_sample(img, t, cond, inits, cond_scale, rescaled_phi, reward_fn=reward_fn)

        img = self.unnormalize(img)
        return img

    @torch.no_grad()
    def ddim_sample(self, shape, cond, inits, cond_scale=1., rescaled_phi=0.0, clip_denoised=True, reward_fn=None):
    ...
    @torch.no_grad()
    def sample(self, cond, no_grad=True, inits=None, cond_scale=1., rescaled_phi=0.0, reward_fn=None):
        batch_size = cond.shape[0]
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        # 将 reward_fn 往下传
        return sample_fn((batch_size, channels, seq_length), cond, inits, cond_scale, rescaled_phi, reward_fn=reward_fn)
    ...
    def p_losses(self, x_start, t, cond, inits, noise=None, mask=None):
        b, c, n = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        model_out = torch.nan_to_num(model_out, nan=0.0, posinf=1e4, neginf=-1e4)
        target = torch.nan_to_num(target, nan=0.0, posinf=1e4, neginf=-1e4)

        sq_err = (model_out - target) ** 2

        if mask is not None:
            m = mask.to(sq_err.dtype)
            if m.dim() == 2:
                m = m.unsqueeze(1)
            if m.size(1) == 1 and sq_err.size(1) > 1:
                m = m.expand(-1, sq_err.size(1), -1)
            sq_err = sq_err * m
            denom = m.sum(dim=(1, 2)).clamp(min=1.0)
            loss = sq_err.sum(dim=(1, 2)) / denom
        else:
            loss = reduce(sq_err, 'b ... -> b', 'mean')

        loss = loss * extract(self.loss_weight, t, loss.shape)
        return loss.mean()

    def forward(self, img, *args, mask=None, **kwargs):
        ...
        img = torch.nan_to_num(img, nan=0.0, posinf=1e4, neginf=-1e4)
        return self.p_losses(img, t, *args, mask=mask, **kwargs)
...
class GaussianDiffusionConditionalTrainer(nn.Module):
    ...
    def p_sample(self, x, t: int, cond, inits, cond_scale=1., rescaled_phi=0.0, clip_denoised=True, reward_fn=None):
        ...
        # 1. 正常的 Unet 预测均值 (mu) 和 方差 (variance)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, t=batched_times, cond=cond, inits=inits, 
            cond_scale=cond_scale, rescaled_phi=rescaled_phi, clip_denoised=clip_denoised
        )
        ...
        if reward_fn is not None:
            ...
            # A. 变异 (Mutation): 在预测的均值附近生成候选种群
            std = torch.exp(0.5 * model_log_variance)
            noise_pop = torch.randn(pop_size, *model_mean.shape[1:], device=device)
            candidates = model_mean.repeat(pop_size, 1, 1) + noise_pop * std * explore_scale
            
            # B. 辅助解码: 假设通道 0 是 dx, 通道 1 是 dy，将其积分为绝对坐标 X, Y
            pred_x = torch.cumsum(candidates[:, 0, :], dim=-1) 
            pred_y = torch.cumsum(candidates[:, 1, :], dim=-1)
            trajs_xy = torch.stack([pred_x, pred_y], dim=1) 
            
            # C. 适应度评估 (Fitness Evaluation): 调用打分函数
            scores = reward_fn(trajs_xy) 
            
            # D. 选择与更新 (Selection & Update)
            k_elite = max(1, int(pop_size * 0.2)) # 选出前 20%
            elite_indices = torch.topk(scores, k=k_elite).indices
            elites = candidates[elite_indices]
    ...
    def p_sample_loop(self, shape, cond, inits, cond_scale=1., rescaled_phi=0.0, reward_fn=None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            # 将 reward_fn 传给单步降噪函数
            img, x_start = self.p_sample(img, t, cond, inits, cond_scale, rescaled_phi, reward_fn=reward_fn)

        img = self.unnormalize(img)
        return img
    def ddim_sample(self, shape, cond, inits, cond_scale=1., rescaled_phi=0.0, clip_denoised=True, starting_data = None, starting_timestep=None, reward_fn=None):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
    ...
    def sample(self, cond, no_grad=False, inits=None, cond_scale=1., rescaled_phi=0.0, reward_fn=None):
        if no_grad:
            with torch.no_grad():
                ...
                sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
                # 传入 reward_fn
                samples = sample_fn((batch_size, channels, seq_length), cond, inits, cond_scale, rescaled_phi, reward_fn=reward_fn)
